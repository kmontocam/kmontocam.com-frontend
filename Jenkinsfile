    ...
    stages {
        stage('LATEX COMPILATION') {
            steps {
                container('tectonic') {
                    sh '''
                        remote_resume_checksum=$(curl -s https://www.kmontocam.com/files/resume.tex | sha256sum | awk '{print $1}')
                        local_resume_checksum=$(sha256sum ./files/resume.tex | awk '{print $1}')
                            ...
                            withEnv([
                                ...
                                buckets.each { bucket ->
                                    sh """
                                        ...
                                        aws s3 cp --recursive --exclude "Makefile" files/ s3://${bucket}/files/
                                        """
                                }
                            }
                        }
